...
export const getAll = async (dispatch, token, curPage, resultPerPage, query) => {
  let url = `/api/faq/all/?keyword=${query}&resultPerPage=${resultPerPage}&currentPage=${curPage}`;
...
export const update = async (dispatch, token, id, faqInfo) => {
  try {
    dispatch({ type: "UPDATE_REQUEST" });

    await axiosInstance.put(`/api/admin/faq/${id}`, faqInfo, {
      headers: { Authorization: token },
    });
